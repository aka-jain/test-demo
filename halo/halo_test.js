...
function loadData() {
    ...
    d3.json("data/data-mock.json", function (json) {

        dataSource.house=json;
        data=json;

        // Once we have our first set of data, we grab our next one
        // We want to make sure both are loaded up before initializing.
        d3.json("data/iphone-data.json", function (json) {
            dataSource.senate = json;
            initialize();
        });
...
function initialize() {
    ...
    viz.data(data)
        ...
        .value(function (d) {
            return Number(d.reviewScore); })    // The property that determines the weight/size of the link path
    ...
    viz_title = viz.selection().select("svg").append("text").attr("class", "title")
        ...
        .attr("x", viz.width() / 2).attr("y", 40).attr("text-anchor", "middle").style("font-weight",300).text("Pixel 2 Review Analysis");
...
function node_onMouseOver(e,d,i) {
    ...
    links.each(function (d) {
        ...
        halos.each(function (d) {
            if (!haloLabels[d.data.key]) {
                ...
                createPacLabel(d.x, d.y,d.data.values[0].reviewText);
                reviewObj.push(d.data.values[0].reviewText);
            }
        })
    });
    ...
    createDataTip(x + d.r, y + d.r + 25, d.values[0].deviceName,"Review Score: " + total, reviewObj, d.values[0].aspect);
}
...
function arc_onMouseOver(e,d,i) {
    ...
    for(var i in d.data.values){
        aspectObj.push(d.data.values[i].aspect)
    }
    ...
    createDataTip(d.x + rect.left  + rect.width/2, (d.y + rect.top + 100),d.data.values[0].deviceName, "Review Score: " + total, [d.data.values[0].reviewText], aspectObj);
}
...
function link_onMouseOver(e,d,i) {
    ...
    //Create and position the datatip
    var rect = d3.selectAll(".vz-halo-arc-plot")[0][0].getBoundingClientRect();
    createDataTip(datum.x + rect.left + datum.r, datum.y + datum.r + 25 + rect.top, d.data.deviceName,  "Review Score: " + total, [d.data.reviewText], [d.data.aspect]);
...
var aspectData='<div class="tooltip aspect">' +
    '<div class="header2"> ASPECT </div>' +
    '</div>';
...
//     var revieEle = document.getElementById('review');
//     revieEle.innerHTML = ''
...
function createDataTip(x,y,h1,h2, reviewsObj, aspectObj) {
    var revieEle = document.getElementById('review');
    var aspectEle = document.getElementById('aspect');
    ...
    html = html.replace("HEADER2", h2);
    var reviewList = '';
    for(var i in reviewsObj){
        var index = parseInt(i) + 1
        reviewList += '<div class="review-list">' + index + '. ' + reviewsObj[i] + '</div>'
    }

    html = html.replace("HEADER3", reviewList);
    ...
    aspectEle.innerText = '';
    var htmlAspect = aspectData.replace("ASPECT", aspectObj);
    d3.select("#aspect")
        .append("div")
        .attr("class", "vz-halo-label")
        .style("opacity",0)
...
function onMouseOut(d,i) {
    var revieEle = document.getElementById('review');
    revieEle.innerHTML = 'Click or hover to view the respective review'
    var revieEle = document.getElementById('aspect');
    revieEle.innerHTML = 'Click or hover on node to view the respective aspect.'
...
var customSkin = {
    ...
    link_stroke: function (d, i) {
        ...
        if(d.data.sentimentId == 1){
            return aspect1;
        } else if(d.data.sentimentId  == 2){
            return aspect2;
        } else if(d.data.sentimentId  == 3){
            return aspect3;
        } else if(d.data.sentimentId  == 4){
            return aspect4;
        }
        // return (d.data.sentimentId == "positive") ? repColor : (d.nodeGroup="negetive") ? demColor: otherColor;
    },
    ...
    node_fill: function (d, i) {
        ...
        if(d.nodeGroup == 1){
            return aspect1;
        } else if(d.nodeGroup == 2){
            return aspect2;
        } else if(d.nodeGroup == 3){
            return aspect3;
        } else if(d.nodeGroup == 4){
            return aspect4;
        }
    },
